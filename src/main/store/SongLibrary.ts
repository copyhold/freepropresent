...
export class SongLibrary {
  ...
  private rebuildReferences(): void {
    ...
    // Sync mainSongId on SongEntry objects — must happen after references are fully built
    // so load order of files doesn't affect correctness.
    for (const [path, entry] of this.songs) {
      const ref = this.references.get(path);
      entry.mainSongId = ref && ref.role !== "main" ? ref.key : undefined;
    }
  }
  ...
  findSongsBySearch(search: string): Set<SongEntry["filePath"]> {
    ...
    for (const [, song] of this.songs) {
      ...
      const mainSongPath = this.references.get(song.filePath)?.key;
      if (!mainSongPath) {
        continue;
      }
      foundPaths.add(mainSongPath);
    }
    return foundPaths;
  }
}
